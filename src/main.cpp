...
#if defined(NDEBUG)
# error "Clickgem cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x65dbe025c092d739045e682155c54a9289f7cd6200a61c34f3f59ba9136c0723");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Clickgem: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Clickgem Signed Message:\n";
...
{
    // Clickgem: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 150000 blocks, which will occur approximately every 416 days
    nSubsidy >>= (nHeight / 150000); // Clickgem: 150000 blocks in ~416 days

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Clickgem: 1 days
static const int64 nTargetSpacing = 2 * 60; // Clickgem: 4 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Clickgem: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    if (vtx[0].GetValueOut() > GetBlockValue(pindex->nHeight, nFees)&&pindex->nHeight>50000)
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Clickgem: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x65dbe025c092d739045e682155c54a9289f7cd6200a61c34f3f59ba9136c0723");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("548364710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1511371417;
        ...
        assert(block.hashMerkleRoot == uint256("0x02e9057b5b1bf36c246538cd0e14501932510e9f6cd94b1d3314c0d8d6dbcb5c"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Clickgem: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ClickgemMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ClickgemMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ClickgemMiner(CWallet *pwallet)
{
    printf("ClickgemMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("clickgem-miner");
    ...
    try { loop {
        ...
        printf("Running ClickgemMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ClickgemMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ClickgemMiner, pwallet));
}
