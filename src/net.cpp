...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Clickgem " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"sv1.clickgem.com","111.90.140.12"},
    {"123.16.15.184", "123.16.15.184"},
    {NULL, NULL}
};
...
void ThreadOpenConnections()
{
    ...
    loop
    {
        ...
        if (addrman.size()==0 && (GetTime() - nStart > 60) && !fTestNet)
        {
            ...
            addrman.Add(vAdd, CNetAddr("111.90.140.12"));
        }
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Clickgem is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(boost::thread_group& threadGroup)
{
    ...
    if (pnodeLocalHost == NULL)
        pnodeLocalHost = new CNode(INVALID_SOCKET, CAddress(CService("111.90.140.12", 0), nLocalServices));
